...
public class ProjectsCombo extends XMLCombo {
  ...
  /**
   * Constructor I for ProjectsCombo.
   * Creates a combo with a value = "null" and text = "Select project".
   *
   *
   * @param    profile                the UserProfile of the user.
   */
  public ProjectsCombo( UserProfile profile )
                throws XMLBuilderException {
    this( profile, "Select project" );
  }
  /**
   * Constructor II for ProjectsCombo.
   ...
   */
  public ProjectsCombo( UserProfile profile, boolean makeNullEntry )
                throws XMLBuilderException {
    super( profile, makeNullEntry );
  }
  /**
   * Constructor III for ProjectsCombo.
   ...
   */
  public ProjectsCombo( UserProfile profile, String nameOnNullEntry )
                throws XMLBuilderException {
  ...
  public void load( String valueToBeSelected )
    ...
    try {
      ...
      if ( userId == -1 ) {
        rs = dbQuery.getAllProjects();
      } else {
        rs = dbQuery.getProjectsForUser( userId );
      }
      while ( !rs.getEOF() ) {
