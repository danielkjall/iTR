    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException {
        ...
        try {
            ...
            if (action != null && action.equalsIgnoreCase("start")) {
                ...
                /*set parameters*/
                String hourValue = request.getParameter("mo");
                if (ItrUtil.isStrContainingValue(hourValue)) {
                    monHours = hourValue.replace(',', '.');
                }

                hourValue = request.getParameter("tu");
                if (ItrUtil.isStrContainingValue(hourValue)) {
                    tueHours = hourValue.replace(',', '.');
                }

                hourValue = request.getParameter("we");
                if (ItrUtil.isStrContainingValue(hourValue)) {
                    wedHours = hourValue.replace(',', '.');
                }

                hourValue = request.getParameter("th");
                if (ItrUtil.isStrContainingValue(hourValue)) {
                    thuHours = hourValue.replace(',', '.');
                }

                hourValue = request.getParameter("fr");
                if (ItrUtil.isStrContainingValue(hourValue)) {
                    friHours = hourValue.replace(',', '.');
                }

                hourValue = request.getParameter("sa");
                if (ItrUtil.isStrContainingValue(hourValue)) {
                    satHours = hourValue.replace(',', '.');
                }

                hourValue = request.getParameter("su");
                if (ItrUtil.isStrContainingValue(hourValue)) {
                    sunHours = hourValue.replace(',', '.');
                }
