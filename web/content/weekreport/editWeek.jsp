...
<%
    synchronized (this) {
        ...
        if (action != null && action.equalsIgnoreCase("start")) {
            ...
            if (ItrUtil.isStrContainingValue(mo)) {
                monHours = mo.replace(',', '.');
            }
            if (ItrUtil.isStrContainingValue(tu)) {
                tueHours = tu.replace(',', '.');
            }
            if (ItrUtil.isStrContainingValue(we)) {
                wedHours = we.replace(',', '.');
            }
            if (ItrUtil.isStrContainingValue(th)) {
                thuHours = th.replace(',', '.');
            }
            if (ItrUtil.isStrContainingValue(fr)) {
                friHours = fr.replace(',', '.');
            }
            if (ItrUtil.isStrContainingValue(sa)) {
                satHours = sa.replace(',', '.');
            }
            if (ItrUtil.isStrContainingValue(su)) {
                sunHours = su.replace(',', '.');
            }
                            ...
                            <table border="0" cellspacing="0" cellpadding="1" bgcolor="#ffffff">
                                <tr>
                                    ...
                                    <td nowrap="true" class="Normal">
                                        <%=aWeek.getFromDate().getYearDisplay()%> <%=aWeek.getFromDate().getMonthNameShort()%>
                                    </td>
        ...
        <form action="editWeek.jsp" name="addARow" enctype="text/plain" method="get" onsubmit="return validateAdd(this)">
            ...
            <tr class="NormalBold">
                ...
                <%
                    ...
                    if (aWeek.getEditRow().isMonWeekend()) {
                        monBgColor = "#FF2222";
                    }
                    if (aWeek.getEditRow().isTueWeekend()) {
                        tueBgColor = "#FF2222";
                    }
                    if (aWeek.getEditRow().isWedWeekend()) {
                        wedBgColor = "#FF2222";
                    }
                    if (aWeek.getEditRow().isThuWeekend()) {
                        thuBgColor = "#FF2222";
                    }
                    if (aWeek.getEditRow().isFriWeekend()) {
                        friBgColor = "#FF2222";
                    }
                    if (aWeek.getEditRow().isSatWeekend()) {
                        satBgColor = "#FF2222";
                    }
                    if (aWeek.getEditRow().isSunWeekend()) {
                        sunBgColor = "#FF2222";
                    }
                %>
            ...
            <tr>
                <td nowrap="true">
                    ...
                    <%
                        if (aWeek.getEditRow().getProject().getProjectCode().length() > 0) {
                            out.write(aWeek.getEditRow().getProject().getProjectCode());
                        }
                        if (aWeek.getEditRow().getProject().getProjectCode().length() > 0) {
                            out.write("-" + aWeek.getEditRow().getProject().getProjectActivity().getCode());
                        }
                    %>
                </td>
                <td>
        ...
        <form action=""  enctype="text/plain" name="saveSubmit" method="POST" onsubmit="return validateSubmit(this)">
            ...
            <tr bgcolor="<%=bgColor%>">
                ...
                <td class="Normal"><%=aRow.getTimeType()%></td>
                    <td align="right" class="Normal"><%if (aRow.isMonExist()) {
                            out.write("" + aRow.getMonday());
                        }%></td>
                    <td align="right" class="Normal"><%if (aRow.isTueExist()) {
                            out.write("" + aRow.getTuesday());
                        }%></td>
                    <td align="right" class="Normal"><%if (aRow.isWedExist()) {
                            out.write("" + aRow.getWednesday());
                        }%></td>
                    <td align="right" class="Normal"><%if (aRow.isThuExist()) {
                            out.write("" + aRow.getThursday());
                        }%></td>
                    <td align="right" class="Normal"><%if (aRow.isFriExist()) {
                            out.write("" + aRow.getFriday());
                        }%></td>
                    <td align="right" class="Normal"><%if (aRow.isSatExist()) {
                            out.write("" + aRow.getSaturday());
                        }%></td>
                <td align="right" class="Normal"><%if (aRow.isSunExist()) {
                        out.write("" + aRow.getSunday());
                    }%></td>
